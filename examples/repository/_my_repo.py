...
class SimpleRepository(Repository):
    ...
    def __init__(self, repo_path : str) -> None:
        ...
        if not repo_path:
            logger.debug("repo_path is None")
            sys.exit('repo_path is Not Defined')
        ...
        if not os.path.isdir(f'{repo_path}/metadata'):
            os.mkdir(f'{repo_path}/metadata')
            
        # Create Directory for Targets
        if not os.path.isdir(f'{repo_path}/targets'):
            os.mkdir(f'{repo_path}/targets')
